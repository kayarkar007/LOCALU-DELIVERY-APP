...
export default async function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    return (
        <div className="space-y-10 animate-in fade-in duration-500">
            <Link href="/" className="inline-flex items-center gap-2 text-sm font-bold text-gray-500 hover:text-gray-900 transition-colors bg-white px-5 py-2.5 border border-gray-200 rounded-full hover:shadow-md w-fit hover:-translate-x-1 duration-300">
                <ChevronLeft className="w-5 h-5" /> Back to Home
            </Link>

            <div className="pb-6 border-b border-gray-100">
                <h1 className="text-4xl md:text-5xl font-black text-gray-900 tracking-tight">{categoryName}</h1>
                <p className={`text-xs font-black uppercase tracking-widest mt-4 inline-block px-3 py-1.5 rounded-lg ${categoryType === "service" ? "bg-purple-100 text-purple-700" : "bg-blue-100 text-blue-700"
                    }`}>
